...
import { Bell, CheckCheck, Bot, FileText, MessageSquare, Package, Star, X } from 'lucide-react'
...
const typeIcon: Record<string, React.ReactNode> = {
  AGENT_TASK_DONE: <Bot size={14} className="text-blue-500" />,
  REQUIREMENT_PENDING: <FileText size={14} className="text-amber-500" />,
  QNA_ANSWERED: <MessageSquare size={14} className="text-green-500" />,
  BUILD_READY: <Package size={14} className="text-purple-500" />,
  FEEDBACK_RECEIVED: <Star size={14} className="text-orange-500" />,
...
export function NotificationBell() {
  ...
  const markRead = async (id: string) => {
    await apiClient.patch(`/api/notifications/${id}/read`).catch(() => null)
    setItems((prev) => prev.map((n) => (n.id === id ? { ...n, read: true } : n)))
    setUnread((c) => Math.max(0, c - 1))
  }

  const markAllRead = async () => {
    await apiClient.patch('/api/notifications/read-all').catch(() => null)
